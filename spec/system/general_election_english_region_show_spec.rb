...
describe "Test for checking page load and data accuracy of general_election/:general_election/countries/2/english-regions/:english-region 2024 07 04 GE", type: :system do
	#have to hard code date chosen
	let(:polling_date)	{'2024-07-04'}
	let(:polling_date_as_display)     { '4 July 2024' } 
	#difficult to check the first line via simple db query so hard coding no changes to be expected to data at this point for 2024 GE
	let(:first_line_of_results_table) {[ 'Labour', '91', '1,067,897','24.5%', '36' ]}
	#			
						

	it 'loads general-election/:general-election/countries/2/english-regions/:english-region based on 2024 07 04 GE' do
		...
		#pass to general_election_list_path
			visit general_election_english_region_show_path(generalelection, country, englishregion)
		...
		#check content on page suggests correct page?
			...
			expect(page).to have_content("#{englishregionName}, #{countryName} - by party")

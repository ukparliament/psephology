task :import_verified_only => [
  :report_start_time,
  :import_constituency_numbers_if_verified,
  :import_candidacy_numbers_if_verified,
  :populate_result_positions_on_candidacies_if_verified,
  :report_end_time
]

# A task to import constituency numbers for verified constituency elections.
task :import_constituency_numbers_if_verified => :environment do
  ...
  CSV.foreach( "db/data/results-by-parliament/#{NEW_PARLIAMENT_NUMBER}/publication-state-tests/part-verified/constituencies.csv"  ).with_index do |row, index|
    
    # ... we skip the first row.
    next if index == 0
    
    # We get the verification state of the election.
    is_verified = row[18]
    
    # If the verification state of the election is verified (TRUE) ...
    if is_verified == 'TRUE'
...
# A task to import candidacy numbers for verified elections.
task :import_candidacy_numbers_if_verified => :environment do
  
  # For each row in the candidacies spreadsheet ...
  CSV.foreach( "db/data/results-by-parliament/#{NEW_PARLIAMENT_NUMBER}/publication-state-tests/part-verified/candidacies.csv"  ).with_index do |row, index|
    ...
    # If the election the candidacy belongs to is verified ...
    if candidacy.election.is_verified
...
# A task to populate result positions on verified elections.
task :populate_result_positions_on_candidacies_if_verified => :environment do
  ...
  general_election.elections.each do |election|
    ...
    # If the election has been verified ...
    if election.is_verified

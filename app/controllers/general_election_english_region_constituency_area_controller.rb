class GeneralElectionEnglishRegionConstituencyAreaController < ApplicationController
  ...
  def index
    ...
    @page_title = "#{@general_election.common_title} - #{@english_region.name}, England - by constituency"
    @multiline_page_title = "#{@general_election.common_title} <span class='subhead'>#{@english_region.name}, England - by constituency</span>".html_safe
    @description = "#{@general_election.common_description} in #{@english_region.name}, England, listed by constituency."

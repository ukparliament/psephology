class ParliamentPeriodController < ApplicationController
  ...
  def index_as_list
    ...
    @page_title = 'Elections by Parliament period'
    @multiline_page_title = "Elections <span class='subhead'>By Parliament period</span>".html_safe
    @description = 'Elections by Parliament period.'
    @csv_url = parliament_period_list_url( :format => 'csv' )
    @crumb << { label: 'Elections by Parliament period', url: nil }
    @section = 'parliament-periods'
  end
